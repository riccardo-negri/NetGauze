...
pub struct RenormalizationStats {
    ...
    pub renormalized_flows: opentelemetry::metrics::Counter<u64>,
    pub processed_flows: opentelemetry::metrics::Counter<u64>,
    pub netflow_v9_not_supported: opentelemetry::metrics::Counter<u64>, /* temporary metric
                                                                         * until v9 is fully
                                                                         * supported */
...
impl RenormalizationStats {
    pub fn new(meter: Meter) -> Self {
        let received_messages = meter
            ...
            .build();
        let sent = meter
            .u64_counter("netgauze.collector.flows.renormalization.sent")
            .with_description("Number of records successfully sent upstream (renormalized or not)")
            .build();
        let send_error = meter
            .u64_counter("netgauze.collector.flows.renormalization.send.error")
            .with_description("Number of renormalized records upstream send error")
            .build();

        let renormalized_flows = meter
            .u64_counter("netgauze.collector.flows.renormalization.renormalized.flows")
            .with_description("Number of flows successfully renormalized")
            .build();
        let processed_flows = meter
            .u64_counter("netgauze.collector.flows.renormalization.processed.flows")
            .with_description(
                ...
                flow = self.rx.recv() => {
                    match flow {
                        Ok((peer, flow)) => {
                            ...
                            self.stats.received_messages.add(1, &tags);

                            // call renormalization processing
                            let renormalized = renormalize(flow, &self.stats, &tags);
