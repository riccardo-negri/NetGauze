...
async fn test_shutdown_command() {
    ...
    let handle = tokio::spawn(async move { actor.run().await }.with_subscriber(subscriber));
    ...
    assert!(logs.contains("Shutting down flow renormalization actor"));
}
...
async fn test_cmd_channel_closed() {
    ...
    let logs = writer.output();
    assert!(logs.contains("Flow renormalization actor terminated due to command channel closing"));
}
...
async fn test_flow_send_error() {
    ...
    let logs = writer.output();
    assert!(logs.contains("Flow renormalization send error"));
...
async fn test_flow_recv_error() {
    ...
    let result = async move { actor.run().await }
        .with_subscriber(subscriber)
        .await;

    assert!(result.is_err());
    let err = result.unwrap_err();
    assert!(err.to_string().contains("error in flow receive channel"));

    // Also verify logs
    let logs = writer.output();
    assert!(logs.contains("Shutting down due to Renormalization recv error"));
}
